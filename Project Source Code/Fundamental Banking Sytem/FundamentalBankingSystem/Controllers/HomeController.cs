...
namespace FundamentalBankingSystem.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            HttpContext.Session.Abandon(); // Destroys all objects stored in a session and releses the resources.
            FormsAuthentication.SignOut(); // clears cookies(authentication ticket) from the browser.
            return View();
        }
                ...
                try
                {
                    ...
                    if (obj != null) // check whether the object returned from DB is not equals to null
                    {
                        ...
                        if (loginid > 0)
                        {
                            ...
                            // append the roleid to the current session
                            Session["roleid"] = obj.roleid;
                            // check whether roleid is 1 or 2
                            if (Session["roleid"].ToString() == "1" && loginid == 1)
                            {
                                // if 1 redirects to manager
                                return RedirectToAction("Manager");
                            }
                            else if (Session["roleid"].ToString() == "2" && loginid == 2)
                            {
